...
int main() {
  ...
  const char *input_text =
      "This is a Test string with   Some extra   spaces! 測試";
  ...
  char **reduced = reduce_text_process(
      PROCESS_TYPE_VARIANT_NORM_DELETE_NORMALIZE, input_text);
  if (reduced) {
  ...
  if (matcher) {
    ...
    // Test process
    SimpleResultList *results =
        simple_matcher_process(matcher, "妳好，這是一個測試句子");
    if (results) {
      printf("simple_matcher_process: %zu match(es)\n", results->len);
      for (size_t i = 0; i < results->len; i++) {
        printf("  [%zu] word_id=%u, word=%s\n", i, results->items[i].word_id,
               results->items[i].word);
      }
      if (results->len == 0) {
    ...
    SimpleResultList *no_results =
        simple_matcher_process(matcher, "nothing here");
    if (no_results) {
    ...
    // Test find_match
    SimpleResult *found =
        simple_matcher_find_match(matcher, "這是一個測試句子");
    if (found) {
    ...
    SimpleResult *not_found =
        simple_matcher_find_match(matcher, "nothing here");
    if (not_found != NULL) {
      fprintf(stderr,
              "Error: expected find_match to return NULL for no match.\n");
    ...
    if (empty != NULL) {
      fprintf(stderr,
              "Error: expected find_match to return NULL for empty text.\n");
  ...
  printf("\n--- Batch tests ---\n");
  {
    ...
    const char *batch_texts[] = {"hello world", "goodbye", "say hello",
                                 "the world"};
    size_t batch_count = 4;

    // batch_is_match
    bool *is_matches =
        simple_matcher_batch_is_match(batch_matcher, batch_texts, batch_count);
    if (!is_matches) {
      fprintf(stderr, "Error: batch_is_match returned null.\n");
      return 1;
    }
    printf("batch_is_match: [%d, %d, %d, %d]\n", is_matches[0], is_matches[1],
           is_matches[2], is_matches[3]);
    ...
    // batch_process
    SimpleResultList *batch_results =
        simple_matcher_batch_process(batch_matcher, batch_texts, batch_count);
    if (!batch_results) {
      fprintf(stderr, "Error: batch_process returned null.\n");
      return 1;
    }
    printf("batch_process: [%zu, %zu, %zu, %zu] matches\n",
           batch_results[0].len, batch_results[1].len, batch_results[2].len,
           batch_results[3].len);
    ...
    if (!batch_found[0] || batch_found[1] || !batch_found[2] ||
        !batch_found[3]) {
    ...
    printf("batch_find_match: [word_id=%u, NULL, word_id=%u, word_id=%u]\n",
           batch_found[0]->word_id, batch_found[2]->word_id,
           batch_found[3]->word_id);
  ...
  bool ok =
      simple_matcher_builder_add_word(builder, PROCESS_TYPE_NONE, 1, "測試");
  if (!ok) {
