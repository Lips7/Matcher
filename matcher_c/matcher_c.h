...
// Returns true if text matches any pattern. Returns false on null input or
// error.
...
SimpleResultList *simple_matcher_process(const void *simple_matcher,
                                         const char *text);

// Returns the first match, or NULL if no match.
// Caller must free a non-NULL return with drop_simple_result.
SimpleResult *simple_matcher_find_match(const void *simple_matcher,
                                        const char *text);
...
// Batch process: matches each text in parallel, returns SimpleResultList array
// of `count`. Caller must free with drop_simple_result_list_array(ptr, count).
SimpleResultList *simple_matcher_batch_process(const void *simple_matcher,
                                               const char **texts,
                                               size_t count);

// Batch find_match: finds first match per text in parallel, returns
// SimpleResult* array. Each element is either a valid pointer (match found) or
// NULL (no match). Caller must free with drop_simple_result_ptr_array(ptr,
// count).
...
// Apply a single ProcessType transform to text. Caller must free result with
// drop_string.
char *text_process(ProcessType process_type, const char *text);

// Apply all ProcessType transforms up to process_type and return
// null-terminated array of variants. Caller must free the result with
// drop_string_array.
