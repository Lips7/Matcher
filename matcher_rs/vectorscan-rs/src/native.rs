...
impl Database {
    pub fn new(patterns: Vec<Pattern>, mode: ScanMode, is_literal: bool) -> Result<Self, Error> {
        let db = wrapper::Database::new(patterns, mode, is_literal)?;
        Ok(Self { db })
    }
}

impl<'db> Scanner<'db> {
    pub fn new(db: &'db Database) -> Result<Self, Error> {
        Ok(Self {
            database: &db.db,
    ...
    pub fn scan<F>(&self, data: &[u8], on_match: F) -> Result<Scan, Error>
    where
