...
//! Engine selection is controlled by the `dfa` feature flag: non-`dfa` builds use
//! `daachorse`, while `dfa` builds use `aho-corasick` in DFA mode for constructed
...
///
/// Non-`dfa` builds use a [`daachorse`] charwise matcher for leftmost-longest matching.
/// With the `dfa` feature, constructed normalization matchers use
/// [`aho_corasick::AhoCorasick`] in DFA mode. The same enum also carries the empty
/// no-op sentinel used for `ProcessType::None`.
///
...
impl MultiCharMatcher {
    ...
    /// Deserializes a precompiled `daachorse` automaton from static bytes.
    ///
    /// Only available without the `dfa` feature. Typically called with bytes from
    /// a build-time artifact generated by `build.rs`.
